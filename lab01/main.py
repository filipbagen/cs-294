...
def main(user_query: str):
    entrypoint_agent_system_message = """
        ...
        Your task is to identify the restaurant name from the prompt "{user_query}" by matching it with 
        one famously known restaurant. Then, you will call the function fetch_restaurant_data and 
        calculate_overall_score to finally return an overall score based on.
    """
    ...
    fetch_message = """
        Expert in finding data and extracting keywords. Good at determining relevant data to fetch, using 
        the function fetch_restaurant_data. Notice, the name of the restaurant might be a little different 
        from the real one. For example, sometimes it use ' ' rather than '-' to concate the words, etc. So 
        be little tolerant for the input name. For example, "Chick-fil-A" could be "Chick fil A" or 
        "Chick_fil_A". In this case, you should try every single possible name to fetch the reviews.
    """

    review_message = """
        You are good at analyzing a list of restaurant reviews and extract food and customer service scores.
    ...
    fetch_reviews_task = f"""
        ...
        use spaces, hyphens, or similar small differences. The output should be in the format:
        '{{"restaurant_name": ["review1", "review2", ...]}}'. Use the function 'fetch_restaurant_data'.
    """
